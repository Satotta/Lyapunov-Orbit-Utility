...
% Make unit circle for plotting
theta_circle = 0:0.01:2*pi;
x_circle = cos(theta_circle);
y_circle = sin(theta_circle);

% Define circle for titan
x_titan = const.Rsecondary/const.R.*x_circle + 1 - const.mu;
y_titan = const.Rsecondary/const.R.*y_circle;

% Define circle for saturn
x_saturn = const.Rprimary/const.R.*x_circle - const.mu;
y_saturn = const.Rprimary/const.R.*y_circle;

% Plot the bodies
figure(4)
hold on
plot(x_saturn, y_saturn, 'Color', '[0.8500 0.3250 0.0980]', 'LineWidth', 2)
plot(x_titan, y_titan, 'Color', '[0.6350 0.0780 0.1840]', 'LineWidth', 2)
...
figure(4)
hold on
legend({'Saturn', 'Titan', 'Lyapunov Orbit', 'Stable', 'Unstable'})
hold off
